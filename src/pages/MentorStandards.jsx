...
export default function MentorStandards() {
  ...
  const logResult = async (idx) => {
    ...
    const passed = confirm('Mark as PASSED? Click OK for Yes, Cancel for No.')
    ...
    setItems(next)
    await setDoc(doc(db,'standards',selected), { items: next }, { merge:true })
  }
      ...
      <div className="bg-white border rounded-xl p-4 space-y-2">
        {items.length ? items.map((it, i) => (
          <div key={it.key || i} className="border rounded p-3">
            ...
            {Array.isArray(it.attempts) && it.attempts.length > 0 && (
              <div className="mt-2 text-sm text-slate-600">
                <div className="font-semibold">Recent attempts:</div>
                <ul className="list-disc pl-6">
                  {it.attempts.slice(0,3).map((a, idx) => (
                    <li key={idx}>{new Date(a.ts).toLocaleString()} — {a.value} {a.passed ? '✅' : '❌'} {a.note ? `• ${a.note}` : ''}</li>
                  ))}
                </ul>
              </div>
            )}
