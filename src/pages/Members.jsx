  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {members.map(m => (
          <div key={m.id} className="border rounded-lg p-3 flex items-center justify-between bg-white">
            <div>
              <div className="font-medium">{m.displayName || 'Firefighter'}</div>
              <div className="text-xs text-slate-500">Tier: {m.tier} • Shift: {m.shift || 'A'} • Committed: {m.isCommitted ? '✅' : '—'}</div>
            </div>
            {isMentor && (
