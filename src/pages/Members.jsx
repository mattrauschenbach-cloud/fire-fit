...
export default function Members() {
  ...
  const setTier = async (uid, tier) => isMentor && updateDoc(doc(db, 'profiles', uid), { tier })
  const setShift = async (uid, shift) => isMentor && updateDoc(doc(db, 'profiles', uid), { shift })
  const toggleCommitted = async (uid, val) => isMentor && updateDoc(doc(db, 'profiles', uid), { isCommitted: val })
  const setRole = async (uid, role) => isMentor && updateDoc(doc(db, 'profiles', uid), { role })
  const setMentorFor = async (uid, mentorId, mentorName) => isMentor && updateDoc(doc(db,'profiles',uid), { mentorId, mentorName })
